...
const acceptor3Port = 3004;
let acceptor3Process: ChildProcess;

const learner1Port = 3005;
let learner1Process: ChildProcess;

const learner2Port = 3006;
...
afterAll(() => {
  ...
  acceptor3Process.kill();
  learner1Process.kill();
  learner2Process.kill();
});


test('one acceptor fails, but consensus is obtained', async () => {
  clientProcess = fork(
    ...
    [
      ...
      ProposerArgumentsParser.stringify({
        accepterUrls: [
          ...
          `${Constants.localUrl}:${acceptor3Port}`,
        ],
        minimumQuorumSize: 2,
      }),
    ],
  ...
  acceptor3Process = fork(
    Constants.acceptorIndexFilePath,
    [
      ArgumentsParser.stringify({ port: acceptor3Port, commands: [] }),
      AcceptorArgumentsParser.stringify({
        doTimeout: false,
