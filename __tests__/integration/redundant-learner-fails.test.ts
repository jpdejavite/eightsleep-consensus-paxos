...
test('redundant learner fails, but consensus is obtained', async () => {
  clientProcess = fork(
  ...
  learner2Process = fork(
    ...
    [
      ...
      LearnerArgumentsParser.stringify({ clientUrl: `${Constants.localUrl}:${clientPort}`, doNotSendResponse: true }),
    ],
  );
