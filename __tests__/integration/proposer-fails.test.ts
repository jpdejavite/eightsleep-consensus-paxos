...
const proposer2Port = 3006;
let proposer2Process: ChildProcess;
...
afterAll(() => {
  ...
  proposer2Process.kill();
});


test('proposer fails, but new consensus is obtained', async () => {
  clientProcess = fork(
  ...
  proposer2Process = fork(
    Constants.proposerIndexFilePath,
    [
      ArgumentsParser.stringify({
        port: proposer2Port, commands: [
          new Command(CommandOption.Sleep, 1000),
          new Command(CommandOption.LookForARequest, ''),
          new Command(CommandOption.Sleep, 2000),
          new Command(CommandOption.LookForARequest, ''),
  ...
  expect(response.data).toStrictEqual({ n: 2, value: 'proposer-3006-2' });
